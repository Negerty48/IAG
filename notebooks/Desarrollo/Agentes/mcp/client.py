...
async def run_client():
    ...
    # Open a real log file to redirect stderr.
    # This guarantees a valid OS file descriptor (fileno) in any virtual environment.
    log_path = os.path.join(script_dir, "math_server_stderr.log")
    stderr_log = open(log_path, "w", encoding="utf-8")
    
    try:
        # Connect to the server using the stdio transport client
        async with stdio_client(server_params, errlog=stderr_log) as (read_stream, write_stream):
